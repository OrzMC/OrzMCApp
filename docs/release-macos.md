...
Actions 会先生成未签名 archive，再对导出的 app 使用 Developer ID 和强化运行时重新签名。复签会使用 Developer ID 证书 SHA-1 identity，先签内部 dylib、framework、XPC 和子 app，最后对最外层 `.app` 做一次不带 `--deep` 的 bundle 签名，确保主可执行文件绑定 `Contents/Info.plist`。这条路径复用了历史上稳定的 CI 流程，同时保留本地默认的直接 Developer ID archive 签名方式。
